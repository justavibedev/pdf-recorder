...
@MainActor struct RecentProjectsView: View {
    ...
    var body: some View {
        Group {
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 220, maximum: 310))], spacing: 12) {
                ForEach(model.recentProjects) { project in
                    VStack(alignment: .leading, spacing: 10) {
                        Button { model.open(URL(fileURLWithPath: project.path)) } label: {
                            VStack(alignment: .leading, spacing: 8) {
                                ZStack {
                                    RoundedRectangle(cornerRadius: 6).fill(StudioTheme.background)
                                ...
                                Text(project.title).font(.system(size: 12, weight: .semibold)).lineLimit(1)
                                Text("\(project.recordedCount) of \(project.pageCount) pages recorded").font(.caption).foregroundStyle(.secondary)
                        ...
                        HStack {
                            ...
                            Spacer()
                            Button { model.togglePin(project) } label: { Image(systemName: project.pinned ? "pin.fill" : "pin") }.help(project.pinned ? "Unpin project" : "Pin project").accessibilityLabel(project.pinned ? "Unpin \(project.title)" : "Pin \(project.title)")
                            Menu { Button("Remove from Recents") { model.forgetRecent(project) } } label: { Image(systemName: "ellipsis") }.menuStyle(.borderlessButton).frame(width: 20).accessibilityLabel("Actions for \(project.title)")
                        }.buttonStyle(.borderless)
                    }.padding(12).studioPanel(cornerRadius: 8)
                }
            }.padding(4)
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            if model.exportScope == .range {
                TextField("For example: 1, 3-5", text: $model.exportRange).textFieldStyle(.roundedBorder)
                Text("Page numbers within “\(model.currentDocument?.title ?? "the current PDF")”.").font(.caption).foregroundStyle(StudioTheme.muted)
            }
            ...
            Grid(alignment: .leading, horizontalSpacing: 20, verticalSpacing: 9) {
                GridRow { Text("Included / skipped"); Text("\(included) / \(skipped)").bold() }
            ...
            }.font(.callout).padding(14).frame(maxWidth: .infinity, alignment: .leading).studioPanel(cornerRadius: 8)
            if included == 0 { Text("Choose a valid range with at least one recorded page.").foregroundStyle(.orange).font(.caption) }
            Text("All-documents export follows the PDF tab order, then page order. Your saved take choices stay unchanged. Notes stay private. File size is an estimate.").font(.caption).foregroundStyle(.secondary)
            HStack { Button("Cancel") { model.exportSummary = false }; Spacer(); Button("Choose Location…") { model.exportSummary = false; model.export() }.buttonStyle(StudioButtonStyle(kind: .primary)).disabled(included == 0) }
        }.padding(24).frame(width: 480).background(StudioTheme.sidebar)
    }
}

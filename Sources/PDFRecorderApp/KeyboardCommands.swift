...
@MainActor struct CommandPaletteView: View {
    ...
    private var commands: [PaletteCommand] {
        var values: [PaletteCommand] = [
            ...
            .init("previous", "Previous Page", "Navigation", key: "Page Up / ←", enabled: model.canNavigate && model.pageIndex > 0) { model.navigate(to: model.pageIndex - 1) },
            .init("next", "Next Page", "Navigation", key: "Page Down / →", enabled: model.canNavigate && model.pageIndex + 1 < (model.manifest?.pages.count ?? 0)) { model.navigate(to: model.pageIndex + 1) },
            .init("first", "First Page", "Navigation", key: "Home", enabled: hasProject && model.canNavigate) { model.navigate(to: 0) },
            .init("last", "Last Page", "Navigation", key: "End", enabled: hasProject && model.canNavigate) { model.navigate(to: (model.manifest?.pages.count ?? 1) - 1) },
